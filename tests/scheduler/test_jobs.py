...
def test_register_jobs_supports_large_heartbeat_interval(monkeypatch) -> None:
    ...
    jobs.register_jobs(scheduler, lambda schedule: None, lambda: None, lambda: None, lambda: None, lambda: None)
    ...
    assert isinstance(scheduler.jobs[5]["trigger"], CronTrigger)
    assert scheduler.jobs[5]["id"] == "daily_slack_summary"